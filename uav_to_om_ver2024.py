...
xyz_rai = [
    [sg.Radio('Для файлов OM - прямоуг. коорд.','OPTYPE2',default = True, key = 'radom'),sg.Radio('Для файлов OM - географ. коорд.','OPTYPE2',default = False, key = 'radom2'),sg.Radio('Для файлов MisPlan','OPTYPE2',default = False, key = 'radmp')],
    ...
    if event == 'new_ascii':
        ...
        if (values['radmp'] == False):
            ...
            for i in range (1, num_lines-1):
                ...
                dif_dist = round(math.sqrt(dif_x*dif_x+dif_y*dif_y),12)
                steps_per_point = int(steps*(dif_dist/sum_dif_dist))
                if (steps*(dif_dist/sum_dif_dist) - float(steps_per_point)) > 0.5:
                    steps_per_point += 1
                x_step = dif_x/steps_per_point
                y_step = dif_y/steps_per_point
                print((steps*(dif_dist/sum_dif_dist)),steps_per_point)
                for j in range(steps_per_point-1):
                    x3 = round(x1+(j+1)*x_step,12)
                    y3 = round(y1+(j+1)*y_step,12)
                    k += 1 
            ...
            el_home_point = r[1]
            el_last_point = r[(num_lines-1)]
            x1 = float(el_last_point.split()[8])
            y1 = float(el_last_point.split()[9])
            x2= float(el_home_point.split()[8])
            y2 = float(el_home_point.split()[9])
            dif_x = (x2-x1)
            dif_y = (y2-y1)
            dif_dist = round(math.sqrt(dif_x*dif_x+dif_y*dif_y),12)
            steps_per_point = int(steps*(dif_dist/sum_dif_dist))
